# train the patnet a bit more, see what you get!
import torch
...
directory = "results/ensemble/modified_patnet"
...
for root, _, files in os.walk(directory):
    ...
    for file in files:
        ...
        if file.endswith(".pth"):
            ...
            with open(os.path.join(root, f"{file.replace('.pth', '')}_report.md"), "r") as report_file:
            ...
            make_cm(
                ...
                name=f"MLP {name.replace('_', ' ')} Extra Training",
                path=f"{root}"
            )
            print(cr)

            plot_results(history, f"{root}", title=f"MLP Additional Training")
