...
import numpy as np
from paper_models.cnn import ArtisanalCNN
import pickle
...
for fold, (train_indices, val_indices) in iterable_splitter:
    ...
    fold_train_dataloader = DataLoader(fold_train_dataset, batch_size=32, shuffle=True)
    fold_val_dataloader = DataLoader(fold_val_dataset, batch_size=32, shuffle=False)
    ...
    model = ArtisanalCNN(**hyper_parameters)
    optimizer = optim.SGD(model.parameters(), lr=1e-06)

    history = train_and_test_model(train_dataloader=fold_train_dataloader, test_dataloader=fold_val_dataloader,
                                   model=model, loss_fn=loss_fn, optimizer=optimizer, epochs=2*60,
                                   device="cpu", verbose=False, early_stopping_lookback=20)

    # save the model
    folder = f"custom_k_folds/fold_{fold}"
    ...
    make_cm(
        ...
        name=f"Custom CNN for Fold {fold}",
        path=folder
    )
    print(cr)

    plot_results(history, folder, title=f"Custom CNN for fold {fold}")

    report = [
        fr"Custom CNN for fold {fold}", "\n", cr, "\n", str(model), "\n"
    ]
    with open(f"{folder}/report.md", "w") as report_file:
        report_file.writelines(report)

    torch.save(best_model, f"{folder}/cnn_{fold}.pth")
...
with open('scores/cnn_scores.pkl', 'wb') as pickle_file:
