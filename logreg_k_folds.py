...
hyper_parameters = {'C': 1, 'kernel': 'rbf'}
...
for fold, (train_indices, val_indices) in iterable_splitter:
    ...
    # save the model
    folder = f"logreg_k_folds/fold_{fold}"
    ...
    make_cm(
        ...
        name=f"Logistic Regression for Fold {fold}",
        path=folder
    )
    print(cr)

    report = [
        fr"Logistic Regressiohn fold {fold}", "\n", str(cr), "\n", f'f1:  {f1}', "\n", f'accuracy: {acc}'
    ]
    with open(f"{folder}/report.md", "w") as report_file:
        report_file.writelines(report)

    with open(f'{folder}/logreg_fold_{fold}.pkl', 'wb') as pickle_file:
