...
for fold, (train_indices, val_indices) in iterable_splitter:
    ...
    classifier = RandomForestClassifier(n_estimators=5_000)
    ...
    X_train = [image.flatten().numpy() for image, _ in fold_train_dataset]
    y_train = [y for _, y in fold_train_dataset]

    X_test = [image.flatten().numpy() for image, _ in fold_val_dataset]
    ...
    # save the model
    folder = f"rf_classifier/fold_{fold}"
    ...
    make_cm(
        ...
        name=f"Random Forest Classifier for Fold {fold}",
        path=folder
    )
    print(cr)

    report = [
        fr"Random Forest fold {fold}", "\n", str(cr), "\n", f'f1:  {f1}', "\n", f'accuracy: {acc}'
    ]
    with open(f"{folder}/report.md", "w") as report_file:
        report_file.writelines(report)

    with open(f'{folder}/rf_fold_{fold}.pkl', 'wb') as pickle_file:
...
with open('scores/rf.pkl', 'wb') as pickle_file:
